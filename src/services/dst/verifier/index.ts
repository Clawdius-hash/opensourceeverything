...
import { stripLiterals, stripRegexLiterals, stripComments, escapeRegExp, wholeWord, detectDeadBranchNeutralization, detectStaticValueNeutralization, resolveMapKeyTaint } from './source-analysis.ts';
...
// ---------------------------------------------------------------------------
// Code quality & encapsulation CWEs (batch: 481-499)
// ---------------------------------------------------------------------------

/**
 * STATIC DETECTABILITY ANALYSIS — Notes on what's fundamentally hard
 * =========================================================================
 *
 * CWE-481 (Assigning Instead of Comparing):
 *   NOT impossible. The pattern `if (x = y)` is syntactically detectable. Most modern
 *   compilers warn about this. We can catch it in code_snapshot with high confidence.
 *   Creative approach: Look for assignment operators inside conditional contexts.
 *
 * CWE-482 (Comparing Instead of Assigning):
 *   HARDER than 481. A standalone `x == y;` as a statement (not in if/while/return) is
 *   the signal. But in code_snapshot fragments, we often lack full statement context.
 *   Creative approach: Look for comparison operators that appear as standalone expressions
 *   (not inside if/while/for/return/variable assignment).
 *
 * CWE-483 (Incorrect Block Delimitation):
 *   STATICALLY DETECTABLE. Indentation-vs-braces mismatch is a strong signal. An if/for/while
 *   without braces followed by multiple indented lines is the classic pattern.
 *
 * CWE-484 (Omitted Break in Switch):
 *   VERY detectable statically. case-without-break is a textbook lint check. The challenge
 *   is distinguishing intentional fallthrough from accidental — we look for fallthrough
 *   comments as the "safe" pattern.
 *
 * CWE-486 (Comparison of Classes by Name):
 *   DETECTABLE in languages with reflection. `getClass().getName().equals()` or
 *   `typeof x === "string"` (wrong in JS for objects) or `instanceof` misuse.
 *   Creative approach: Flag string-based type comparisons when reflection-safe alternatives exist.
 *
 * CWE-489 (Active Debug Code):
 *   VERY detectable. Debug statements, debugger keywords, console.log in production code,
 *   debug flags left enabled. Similar to CWE-215 but broader — not just sensitive data in
 *   debug, but the presence of debug code itself.
 *
 * CWE-491 (Public cloneable() Without Final):
 *   JAVA-SPECIFIC but detectable. Look for classes implementing Cloneable without final.
 *   In the DST model, we scan for clone-related patterns in code_snapshot.
 *   Creative approach: Also flag deep-copy methods that don't protect mutable internal state.
 *
 * CWE-495 (Private Data Structure Returned From Public Method):
 *   DETECTABLE with structural analysis. A method returning `this.internalArray` directly
 *   (without copy) is the pattern. We look for return statements that reference private fields.
 *
 * CWE-496 (Public Data Assigned to Private Array-Typed Field):
 *   MIRROR of 495. A setter/constructor that assigns an external array directly to an internal
 *   field without copying. Detectable by looking for `this.field = param` patterns with arrays.
 *
 * CWE-499 (Serializable Class Containing Sensitive Data):
 *   DETECTABLE. Look for classes that implement Serializable/JSON serialization that contain
 *   fields named password, secret, token, etc. without transient/JsonIgnore annotations.
 *
 * VERDICT: None of these 10 are fundamentally impossible to detect statically. They're all
 * syntactic/structural patterns, not semantic/runtime behaviors. The challenge is precision
 * (avoiding false positives) rather than possibility. CWE-482 is the hardest because
 * distinguishing "intentional no-op comparison" from "meant to assign" requires intent analysis.
 */
...
function verifyCWE749(map: NeuralMap): VerificationResult {
  ...
  for (const ext of externals) {
    const code = stripComments(ext.analysis_snapshot || ext.analysis_snapshot || ext.code_snapshot);
...
/* REMOVED: verifyCWE780 -- now in crypto.ts */
...
// ---------------------------------------------------------------------------
// Comparison, concurrency, regex, and hook validation CWEs
// ---------------------------------------------------------------------------
...
/* REMOVED: verifyCWE913 -- now in auth.ts */
...
/* REMOVED: verifyCWE915 -- now in auth.ts */
...
function verifyCWE921(map: NeuralMap): VerificationResult {
  ...
  for (const store of storageNodes) {
    ...
    const code = stripComments(store.analysis_snapshot || store.analysis_snapshot || store.code_snapshot);
...
function verifyCWE922(map: NeuralMap): VerificationResult {
  ...
  for (const store of storNodes922) {
    ...
    if (SENS922.test(combined) && !ENC922.test(stripComments(store.analysis_snapshot || store.analysis_snapshot || store.code_snapshot))) {
...
function verifyCWE208(map: NeuralMap): VerificationResult {
  ...
  for (const src of authNodes208) {
    if (SAFE_COMPARE_RE.test(src.analysis_snapshot || src.analysis_snapshot || src.code_snapshot)) continue;
    if (!SECRET_COMPARE_RE.test(src.analysis_snapshot || src.analysis_snapshot || src.code_snapshot)) continue;
...
function verifyCWE248(map: NeuralMap): VerificationResult {
  ...
  for (const egress of egressNodes) {
    const code = stripComments(egress.analysis_snapshot || egress.analysis_snapshot || egress.code_snapshot);
  ...
  for (const node of map.nodes) {
    ...
    if (match) {
      ...
      if (!ERROR_CHECK.test(code)) {
        ...
        const containerChecks = container ? ERROR_CHECK.test(stripComments(container.analysis_snapshot || container.analysis_snapshot || container.code_snapshot)) : false;
        if (!containerChecks) {
          findings.push({
            source: nodeRef(node), sink: nodeRef(node),
...
/* REMOVED: verifyCWE612 — now in sensitive-data.ts */
...
const CWE_REGISTRY: Record<string, (map: NeuralMap) => VerificationResult> = {
  ...
  // Memory safety & arithmetic CWEs — override generic generated verifiers
  ...
  // Code quality & encapsulation CWEs
  ...
  // Concurrency, temp file, search path, and resource exposure CWEs -- now in resource.ts
  ...
  'CWE-98': verifyCWE98,
  'CWE-99': verifyCWE99,
  // Resource management, consumption, cleanup CWEs -- now in resource.ts
  // Encoding, neutralization, and validation ordering CWEs
  ...
  // Predictability, UI security, trust boundary CWEs
  'CWE-341': verifyCWE341,
  'CWE-342': verifyCWE342,
  'CWE-343': verifyCWE343,
  'CWE-344': verifyCWE344,
  // Race conditions, timing, state CWEs -- now in resource.ts
  ...
  // Logging, crypto padding, trust decisions, infinite loops CWEs
  ...
  // Comparison bugs, concurrency, regex, and hook validation CWEs
  ...
  // Upload variable, variable extraction, null byte, dynamic eval, fail-open CWEs
  'CWE-616': verifyCWE616,
  'CWE-621': verifyCWE621,
  'CWE-626': verifyCWE626,
  'CWE-627': verifyCWE627,
  'CWE-636': verifyCWE636,
  // Encoding, initialization, hidden functionality, mass assignment, and resource CWEs
  ...
  // Regex, integer coercion, and data representation CWEs
  ...
  // Side channel, error handling & information exposure CWEs
  'CWE-207': verifyCWE207,
  'CWE-208': verifyCWE208,
  // Resource cleanup, return values, privilege CWEs
  ...
  // Error handling and resource management CWEs
  ...
  // CWE-401, CWE-403 -- now in resource.ts
  ...
  // Object model, finally, memory safety, API portability CWEs
  ...
  // Port binding, search index authorization, session expiration
